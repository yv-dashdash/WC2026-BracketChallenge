...
app.get('/api/scores', async (_req, res) => {
  try {
    ...
    // MANDATORY: Explicitly use 'live_' data to ensure leaderboard matches admin panel
    const getLive = (key) => actual['live_' + key] || null;

    const scores = users.map(user => {
      const up = allPreds.filter(p => p.user_id === user.id);

      const r32Set = getLive('r32');
      let groups = 0;
      if (r32Set) {
      ...
      const champSet = getLive('champion');
      let champion = 0;
      if (champSet) {
